 ...
 */
public class SnaByteArraySerializer extends AbstractPooledBufferByteArraySerializer {

    public SnaByteArraySerializer(){
        super();
    }

    public SnaByteArraySerializer(int maxMessageSize){
        super();
...
//        SystemLogger.infoMethod(getClass(), "serialize", true, new String[] { "bytes.length" }, bytes.length);
        try {
            ...
            SystemLogger.debug("System.arraycopy 1");
            System.arraycopy(bytes, 0, totalBytes, lengthBytes.length, bytes.length);
            SystemLogger.debug("System.arraycopy 2");
            outputStream.write(totalBytes);

        } catch (Exception e) {
            SystemLogger.error("Occurs an Error while serialize message, e.message={0}", new String[] { e.getMessage() }, e);
            throw e;
        } finally {
//            SystemLogger.infoMethod(getClass(), "serialize", false, new String[] {});
    ...
    public int getBytesOld(InputStream inputStream, byte[] buffer) throws IOException {
        SystemLogger.debugMethod(getClass(), "getBytesOld", true, new String[] {});
        int n = 0;
        try {
            n = inputStream.read(buffer);
            SystemLogger.debug("message length={0}", n);
            if (n < 0) {
            ...
            SystemLogger.debugMethod(getClass(), "getBytesOld", false, new String[] {});
        }
    }

    public int getBytesLen(InputStream inputStream,byte[] buffer) throws IOException {
...
//            System.out.println("Client-targetMessageLength: " + targetMessageLength + "realMessageLength: " + realMessageLength);
...
//                System.out.println("ClientReceivedMessageCnt: " + receivedMessageCnt);
                return checkIntegrityAndPopulateBuffer(inputStream, targetMessageLength, receivedMessageCnt, buffer);
            } else {
