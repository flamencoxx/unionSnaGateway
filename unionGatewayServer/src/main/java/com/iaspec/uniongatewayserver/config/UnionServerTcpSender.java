...
public class UnionServerTcpSender implements TcpSender {
    ...
    public void addNewConnection(TcpConnection con) {
        try {
            ...
            int localPort = con.getSocketInfo()
                    .getLocalPort();
            SystemLogger.info("UMPS  connect to Union Gateway Server, UMPS : {0} : {1} connect to Gateway: {2} : {3},  connectId : {4}",GatewayConstant.SERVER_REMOTE_HOST,GatewayConstant.SERVER_REMOTE_PORT,localHost,localPort,GatewayConstant.IP_CONNECTION_ID);
    ...
    public void removeDeadConnection(TcpConnection con) {
        ...
        try {
            ...
            int localPort = con.getSocketInfo()
                    .getLocalPort();
            SystemLogger.info("UMPS and gateway Server disconnect, UMPS : {0} : {1} disconnect with Gateway: {2} : {3},  connectId : {4}",GatewayConstant.SERVER_REMOTE_HOST,GatewayConstant.SERVER_REMOTE_PORT,localHost,localPort,GatewayConstant.IP_CONNECTION_ID);
